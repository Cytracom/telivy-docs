...
The check runs every time the agent scans (hourly), so coverage is continuous, not point-in-time.
...
- **Cyber insurance gating.** A growing share of carriers require attestation that endpoints are encrypted. "We assumed BitLocker was on" is not an attestation. A continuous Telivy report is.
...
<AccordionGroup>
  <Accordion title="Windows (BitLocker)">
    On Windows, the Telivy agent reads BitLocker volume status directly from the OS — the same data surface as `Get-BitLockerVolume` and the BitLocker WMI provider. For each volume the agent captures:

    | Field | What it means |
    | --- | --- |
    | `ProtectionStatus` | `0` = Off, `1` = On, `2` = Unknown |
    | `LockStatus` | `0` = Unlocked, `1` = Locked |
    | `EncryptionMethod` | Cipher in use (see table below) |
    | `EncryptionPercentage` | How much of the volume is encrypted (0–100) |
    | `KeyProtector` | Which protectors are configured (TPM, recovery password, etc.) |
    | `MountPoint` | Drive letter (e.g. `C:`, `D:`) |
    | `CapacityGB` | Volume size |

    Recognized encryption methods on Windows:
    ...
    A volume that is mid-encryption (e.g. `EncryptionPercentage = 47`) with `ProtectionStatus = 1` is treated as encrypted. A volume that is 100% encrypted but has `ProtectionStatus = 0` (protection turned off) is **not** treated as encrypted — the data is still recoverable from the suspended state.
  </Accordion>
...
When a device is remediated, the next agent scan will close the finding automatically — no manual reconciliation needed.
...
<Warning>
  **Hyper-V hosts and certain virtual volumes can trip this rule.** A Hyper-V host whose physical disk is encrypted at the volume level may still expose virtual volumes (or pass-through disks) that report `ProtectionStatus = 0`, generating a finding even though the underlying data is protected. We're aware of this and tracking the fix — until then, treat findings on hypervisor hosts as needing human verification before remediation.
</Warning>
...
- **Partial encryption is still encryption.** A volume showing `EncryptionPercentage = 47` with active protection is treated as encrypted. The finding is the *active protection* signal, not the percentage.
...
Telivy reads native OS encryption state on every Windows and Mac device, raises a Critical finding for any volume that isn't actively protected, and re-checks every hour. Use the finding list to drive a focused, defensible remediation sprint — and double-check Hyper-V hosts before you act.
